...
def get_product_details(product_url: str) -> dict: # type: ignore
    ...
    try:
        ...
        title = (
            soup.find('span', attrs={'id': 'productTitle'}).get_text().strip()
        )
        extracted_price = (
            soup.find('span', attrs={'class': 'a-price'}).get_text().strip()
        )
